<script lang="ts">
  ...
  <div class="page-content {isBreak ? 'break-time' : ''}">
    ...
    {#if type === 'Document'}
      ...
      <div class="flex justify-center">
        <button class="hover:backdrop-brightness-50 text-white font-regular py-2 px-4 rounded backdrop-brightness-75 py-2 px-5 rounded-lg" on:click={() => saveDocumentContent(documentContent)}>Save</button>
      </div>
    {/if}
